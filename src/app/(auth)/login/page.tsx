...
export default function LoginPage() {
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      if (isNew) {
        toast.success(`Akun Google baru dibuat sebagai donor. Selamat datang, ${profile?.name ?? user.email}!`);
      } else {
        toast.success(`Selamat datang kembali, ${profile?.name ?? user.email}!`);
      }
      ...
      if (msg.includes('popup-closed-by-user')) {
        ...
        toast.error(`Gagal login Google: ${msg}`);
      }
    } finally {
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-8">
          ...
          <button
            ...
            className="w-full inline-flex items-center justify-center gap-3 border border-gray-300 rounded-lg px-4 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 disabled:opacity-50"
          >
